...
public class ComputerTest extends TestBase {
    ...
    public void verifyProductArrangeInAlphabeticalOrder() {
        ...
        List<Double> beforeNameZtoAList = new ArrayList<>();
        for (WebElement nameZtoA : beforeFilterNameZtoAList) {
            beforeNameZtoAList.add(Double.valueOf(nameZtoA.getText().replace("$", " ")));
        }
        ...
        List<Double> afterNameZtoAList = new ArrayList<>();
        for (WebElement nameZtoA : afterFilterNameZtoAList) {
            afterNameZtoAList.add(Double.valueOf(nameZtoA.getText().replace("$", " ")));
        }

        //1.4 Verify the Product will arrange in Descending order.
        Collections.sort(beforeNameZtoAList);
    ...
    public void verifyProductAddedToShoppingCartSuccessFully() throws InterruptedException {
        ...
        String actualText4 = computerPage.getTotalText();
        Assert.assertEquals(actualText4, "$2,950.00", "error");
        ...
        String actualText5 = computerPage.getWelcomePageSignInText();
        Assert.assertEquals(actualText5, "Welcome, Please Sign In!", "error");
        ...
        String actualMessage = computerPage.getCreditCardText();
        Assert.assertEquals(actualMessage, "Credit Card", "error");

        // 2.32 Verify “Shipping Method” is “Next Day Air”
        String actualMessage1 = computerPage.getNextDayAirText();
        Assert.assertEquals(actualMessage1, "Next Day Air", "error");

        //2.33 Verify Total is “$2,950.00”
        String actualText6 = computerPage.getTotalAmountText();
        Assert.assertEquals(actualText6, "$2,950.00", "error");

        //2.34 Click on “CONFIRM”
        computerPage.clickOnConfirmButton();

        //2.35 Verify the Text “Thank You”
        String actualText7 = computerPage.getThankYouText();
        Assert.assertEquals(actualText7, "Thank you", "error");

        // 2.36 Verify the message “Your order has been successfully processed!”
        String actualText8 = computerPage.getOrderSuccessProcessText();
        Assert.assertEquals(actualText8, "Your order has been successfully processed!", "error");

        // 2.37 Click on “CONTINUE”
        computerPage.clickOnContinueButton3();

        // 2.38 Verify the text “Welcome to our store”
        String actualText9 = computerPage.getWelcomeOurStoreText();
        Assert.assertEquals(actualText9, "Welcome to our store", "error");
    }
